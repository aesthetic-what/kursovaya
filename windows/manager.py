...
class ManagerWindow(QMainWindow):
    def __init__(self):
        ...
        self.ui.tableWidget.setHorizontalHeaderLabels(["ID", "Название", "Описание", "Цена", "Поставщик", "Количество","Тип продукта", "Дата добавления\обновления"])
    ...
    def load_products(self):
        ...
        for row, product in enumerate(products):
            ...
            self.ui.tableWidget.setItem(row, 3, QTableWidgetItem(product.price))
            self.ui.tableWidget.setItem(row, 4, QTableWidgetItem(product.importer))
            self.ui.tableWidget.setItem(row, 5, QTableWidgetItem(str(product.quantity)))
            self.ui.tableWidget.setItem(row, 6, QTableWidgetItem(product.type))
            self.ui.tableWidget.setItem(row, 7, QTableWidgetItem(str(product.date_add)))
        session.close()
    ...
    def edit_product(self):
        ...
        prod_name = self.ui.tableWidget.item(selected_row, 1).text()

        # Передаём в окно редактирования
        self.edit_prod_win = EditProductsWindow(prod_name)
    ...
    def search_products(self):
        ...
        for row, product in enumerate(products):
            ...
            self.ui.tableWidget.setItem(row, 4, QTableWidgetItem(product.importer))
            self.ui.tableWidget.setItem(row, 5, QTableWidgetItem(str(product.quantity)))
            self.ui.tableWidget.setItem(row, 6, QTableWidgetItem(str(product.type)))            
            self.ui.tableWidget.setItem(row, 7, QTableWidgetItem(str(product.date_add)))
        session.close()
